...
import { getAllEvents, loadAllEvents } from '../lib/event-queries.js';
...
const gradeConfig = {
  ...
  '4': { label: 'Lớp 4', subtitle: 'Làm quen với lịch sử qua những truyền thuyết và câu chuyện đầu tiên về cội nguồn dân tộc.', image: '/images/generated/hung-vuong-ceremony.png' },
...
export const GradeFilterPage = () => {
  ...
  const events = useMemo(() => applyListingState(allEvents, state), [allEvents, grade, state.sort, state.topic, state.type]);

  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    loadAllEvents().then((loaded) => {
      if (!cancelled) setAllEvents(loaded);
    }).finally(() => {
