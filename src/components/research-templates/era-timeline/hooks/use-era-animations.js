...
/**
 * Era Timeline — Hero entrance animation (same as battle).
 */
...
/**
 * Era Timeline — Section scroll animations.
 */
export function useSectionAnimations(containerRef) {
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      /* Era cards — alternating left/right slide-in */
      gsap.utils.toArray('.era-card').forEach((card, i) => {
      ...
      /* Era event nodes — stagger entrance */
      gsap.utils.toArray('.era-event-node').forEach((node) => {
        gsap.from(node, {
      ...
      /* Era connectors — grow animation */
      gsap.utils.toArray('.era-connector').forEach((conn) => {
        gsap.from(conn, {
          scaleY: 0, opacity: 0, duration: 0.8, ease: 'power3.out',
